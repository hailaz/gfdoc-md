...
格式符输出内容`%v`, `%s`打印所有的层级错误信息，构成完成的字符串返回，多个层级使用 `:` 拼接。`%-v`, `%-s`打印当前层级的错误信息，返回字符串。`%+s`打印完整的堆栈信息列表。`%+v`打印所有的层级错误信息字符串，以及完整的堆栈信息，等同于 `%s\n%+s`。
...
堆栈模式是否默认说明`brief`是**简略模式**。错误堆栈打印时，不会打印框架相关的堆栈。`detail`
