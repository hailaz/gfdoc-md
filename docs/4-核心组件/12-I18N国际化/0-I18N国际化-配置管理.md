...
`gi18n` 国际化组件支持框架通用的五种配置文件格式： `xml/ini/yaml/toml/json/properties`（更详细的文件支持列表具体以配置管理章节为主： [配置管理](/docs/核心组件/配置管理/配置管理)）。同样的，和配置管理模块一样，框架推荐使用 `toml` 文件格式。
...
`gi18n` 默认支持资源管理器（详情请参考章节： [资源管理](/docs/核心组件/资源管理/资源管理)），默认情况下会从 `gres` 配置管理器中检索 `manifest/i18n` 及 `i18n` 目录，或者开发者设置的 `i18n` 目录路径。