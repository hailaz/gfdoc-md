...
函数说明格式示例备注`plus`**加**`{{.value1 | plus .value2}}``{{2 | plus 3}} => 5``3+2``minus`**减**`{{.value1 | minus .value2}}``{{2 | minus 3}} => 1``3-2``times`**乘**`{{.value1 | times .value2}}``{{2 | times 3}} => 6``3*2``divide`**除**`{{.value1 | divide .value2}}``{{2 | divide 3}} => 1.5``3/2`
...
将 `timestamp` 时间戳变量进行时间日期格式化，类似PHP的 `date` 方法， `format` 参数支持 [PHP date](http://php.net/manual/en/function.date.php) 方法格式，亦可参考 [时间管理-gtime](/docs/组件列表/系统相关/时间管理-gtime/时间管理-gtime) 。
...
函数说明格式`json`将模板变量转换为 `JSON` 格式字符串。`{{json .var}}``xml`将模板变量转换为 `XML` 格式字符串。`{{xml .var}}``ini`将模板变量转换为 `INI` 格式字符串。`{{ini .var}}``yaml`将模板变量转换为 `YAML` 格式字符串。`{{yaml .var}}``yamli`将模板变量转换为带有自定义缩进的 `YAML` 格式字符串。`{{yamli .var .indent}}``toml`将模板变量转换为 `TOML` 格式字符串。`{{toml .var}}`